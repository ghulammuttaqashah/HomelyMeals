        ...
        <div className="w-full max-w-md">
          <div className="rounded-lg bg-white p-5 sm:p-8 shadow-sm border border-gray-200">
            <div className="mb-6 sm:mb-8 text-center">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-orange-600">Verify Your Email</h1>
              {pendingEmail && (
                <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-gray-600">
                  We sent a 6-digit code to <span className="font-medium text-gray-900 break-all">{pendingEmail}</span>
                </p>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
              <div>
                <label htmlFor="otp-inputs" className="mb-2 sm:mb-3 block text-xs sm:text-sm font-medium text-gray-700">
                  Enter verification code
                </label>
                <div id="otp-inputs" className="flex gap-1.5 sm:gap-3" onPaste={handlePaste}>
                  {otp.map((digit, index) => (
                    <input
                      key={index}
            ...
            <div className="mt-5 sm:mt-6 text-center">
              <p className="mb-2 sm:mb-3 text-xs sm:text-sm text-gray-600">Didn't receive the code?</p>
              <button
                type="button"
                onClick={handleResend}
                disabled={resending || loading}
                className="text-xs sm:text-sm font-semibold text-orange-600 disabled:cursor-not-allowed disabled:text-gray-400"
              >
                {resending ? (
                  <span className="flex items-center justify-center gap-2">
                    <Loader size="sm" />
